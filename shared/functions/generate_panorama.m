...
%save '../data/data_kth_large_slarv.mat' data; % for later use 
...
normalized_homographies = cell(3,1);
for j = 1:am_cams
    p_ref = data_norm(3*ref_view-2:3*ref_view, ~isnan(data_norm(3*j,:)));
    p_j = data_norm(j*3-2:j*3,~isnan(data_norm(3*j,:)));
    normalized_homographies{j,1} = det_homographies(p_ref,p_j);
    N_j = norm_mat(j*3-2:j*3,:);
    N_ref = norm_mat(ref_view*3-2:ref_view*3,:);
    homographies{j,1} = inv(N_ref)*normalized_homographies{j,1}*N_j;
end

% for j = 1:am_cams
%     homographies{j,1} = det_homographies(data(3*ref_view-2:3*ref_view,...
%         ~isnan(data(3*j,:))),...
%         data(j*3-2:j*3,~isnan(data(3*j,:))));
% end
