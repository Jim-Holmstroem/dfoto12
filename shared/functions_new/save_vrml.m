...
triang_shifted = triang-1;
for row = 1:size(triang,1)
    fprintf(fid,'%i %i %i -1, \n',triang_shifted(row,:));
end
...
ref_data = data(3*para2-2:3*para2,:);
ref_data(1,:) = ref_data(1,:)/800;
ref_data(2,:) = (600-ref_data(2,:))/600;
for p = 1:size(ref_data,2)
    fprintf(fid, '%f %f, \n', ref_data(1:2,p));
end
